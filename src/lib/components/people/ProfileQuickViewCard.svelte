...
<script>
    import { formatJourneyStatus, formatLeadershipRole } from "$lib/services/peopleService.js";
    let { person = null, onclick = null } = $props();

    // Get status badge color
    function getStatusColor(status) {
        const colorMap = {
            contact: "bg-warning/15 text-warning",
                ...
                <div class="text-xs text-muted-foreground space-y-0.5">
                    {#if person.role && person.role !== "no_role"}
                        <div class="flex items-center gap-1">
                            ...
                            <span>Leadership: {formatLeadershipRole(person.role)}</span>
                        </div>
                    {/if}

                    {#if person.church_role === "basonta" || person.basontas?.length}
                        <div class="flex items-center gap-1">
                            <span>Basonta / group involvement: {person.basontas?.length ? person.basontas.map((group) => group.replaceAll("_", " ")).join(", ") : "Basonta member"}</span>
                        </div>
                    {/if}
