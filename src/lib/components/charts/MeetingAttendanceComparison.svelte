<script>
  ...
  $effect(() => { if (!granularityManuallySet) granularity = /year/i.test(periodLabel) ? "month" : "day"; });
  const options = $derived(series.map(item => ({key: String(item.id), label:item.label})));
  $effect(() => {
    if (!options.some(option => option.key === primaryKey)) primaryKey = options[0]?.key || '';
    if (!initialised && options.length) { comparisonKey = options[1]?.key || ''; initialised = true; }
    if (comparisonKey && !options.some(option => option.key === comparisonKey)) comparisonKey = '';
  });
  const selectedSeries = $derived([{key:primaryKey,mode:primaryMode}, ...(comparisonKey ? [{key:comparisonKey,mode:comparisonMode}] : [])].flatMap((selection,index) => {
    const item = series.find(item => String(item.id) === selection.key);
    return item ? [{...item, role:index === 0 ? 'A' : 'B', mode:selection.mode, measureLabel: granularity === 'day' ? 'actual attendance' : selection.mode === 'total' ? 'total attendance' : 'average attendance per meeting', rawPoints:item.points || [], points:groupChartPoints(item.points || [],granularity,selection.mode === 'total' ? 'sum' : 'average'),color:colors[index]}] : [];
  }));
  const summaryPoints = $derived(series.filter(item => [primaryKey,comparisonKey].includes(String(item.id))).flatMap(item => item.points || []));
  ...
  function linePath(points) {
    return makeSmoothCurve(points, "y");
  }
...
<section class="card-base fullscreen-chart overflow-visible p-5" aria-labelledby="meeting-comparison-title">
  <header class="mb-4 flex flex-col gap-3 pr-12">
    ...
    <div class="flex min-w-0 flex-wrap items-start gap-2">
      ...
      <ComparisonControls {options} bind:primaryKey bind:comparisonKey bind:primaryMode bind:comparisonMode averageLabel="Average per meeting" primaryLabel="Series A meeting type" comparisonLabel="Series B meeting type" />
      {#if onFilterClick}
  ...
  {#if selectedSeries.length}
    <div class="mb-2 flex flex-wrap justify-center gap-x-5 gap-y-2 text-xs">
      {#each selectedSeries as item}<span class="flex items-center gap-1.5"><span class="h-2.5 w-2.5 rounded-full" style="background: {item.color}"></span>Series {item.role}: {item.label} · {item.measureLabel}</span>{/each}
    </div>
    <div class="relative">
      <svg viewBox="0 0 {chart.width} {chart.height}" class="fullscreen-chart-svg h-auto w-full overflow-visible" style="height: {chart.height}px;" role="img" aria-label="Meeting attendance comparison chart">
        ...
        {/each}
        {#each chart.dates as date}<text x={chart.x(date)} y={chart.height - 16} text-anchor="middle" class="fill-muted-foreground text-[11px]">{chart.labels[date] || formatDate(date)}</text>{/each}
        {#each chart.plotted as item, seriesIndex}
          {#if chartType === "line"}
            ...
            {#each item.plottedPoints as point}
              <circle cx={point.x} cy={point.y} r="5" fill="hsl(var(--card))" stroke={item.color} stroke-width="3" class="cursor-pointer" role="button" tabindex="0" aria-label={`${item.label}, ${point.label || formatDate(point.date)}, ${point.total} ${item.measureLabel}`} onmouseenter={() => (hoveredPoint = { ...point, seriesLabel: item.label, measureLabel: item.measureLabel })} onmouseleave={() => (hoveredPoint = null)} onfocus={() => (hoveredPoint = { ...point, seriesLabel: item.label, measureLabel: item.measureLabel })} onblur={() => (hoveredPoint = null)} onclick={(event) => inspect(point, item, event)} onkeydown={(event) => { if (event.key === 'Enter' || event.key === ' ') inspect(point, item, event); }} />
            {/each}
          {:else}
            {#each item.plottedPoints as point}
              ...
              <rect x={barX} y={point.y} width={barWidth} height={barHeight} rx="5" fill={item.color} fill-opacity="0.9" class="cursor-pointer transition-all hover:opacity-100" role="button" tabindex="0" aria-label={`${item.label}, ${point.label || formatDate(point.date)}, ${point.total} ${item.measureLabel}`} onmouseenter={() => (hoveredPoint = { ...point, seriesLabel: item.label, measureLabel: item.measureLabel })} onmouseleave={() => (hoveredPoint = null)} onfocus={() => (hoveredPoint = { ...point, seriesLabel: item.label, measureLabel: item.measureLabel })} onblur={() => (hoveredPoint = null)} onclick={(event) => inspect(point, item, event)} onkeydown={(event) => { if (event.key === 'Enter' || event.key === ' ') inspect(point, item, event); }} />
      ...
      {#if hoveredPoint}<div class="pointer-events-none absolute z-30 min-w-[180px] -translate-x-1/2 rounded-xl border border-border/90 bg-card/95 p-3 text-xs shadow-2xl backdrop-blur-md" style="left: clamp(100px, {(hoveredPoint.x / chart.width * 100).toFixed(1)}%, calc(100% - 100px)); top: {Math.max(12, hoveredPoint.y - 92)}px;"><p class="font-semibold text-foreground">{hoveredPoint.seriesLabel}</p><p class="mt-1 text-muted-foreground">{formatDate(hoveredPoint.date)}</p><div class="mt-2 flex items-center justify-between border-t border-border/50 pt-2"><span class="text-muted-foreground">{hoveredPoint.measureLabel}</span><strong class="text-foreground">{hoveredPoint.total}</strong></div><p class="mt-2 text-primary">Click for details</p></div>{/if}
    </div>
