...
{#snippet list(choice, navigate)}
  ...
  <p class="mb-4 font-medium">{choice.metricKey === 'attendance' && choice.mode === 'average' ? result.denominator ? `${result.attendanceTotal} attendances across ${result.denominator} held meetings = ${result.displayed} per meeting` : 'Average unavailable: no held meetings in this selection.' : `${result.total} ${choice.metricKey === 'uniquePeople' ? 'distinct linked people' : choice.metricKey === 'meetingCount' ? 'held meetings' : 'recorded contributions'} across ${result.denominator} held meetings`}</p>
  ...
  {/if}
  {#if choice.value != null && Number(choice.value) !== result.displayed}<p class="mt-3 text-sm text-warning">The chart displays {choice.value}; current source records calculate {result.displayed}.</p>{/if}
{/snippet}
