...
<script>
    ...
    // Collapsed state for months - default to ALL collapsed for progressive disclosure
    let collapsedMonths = $state({});

    // Check if a month is collapsed (default = true for all)
    function isCollapsed(key) {
        // If not explicitly set, default to collapsed (true)
        return collapsedMonths[key] !== false;
    }

    // Toggle month collapse
    function toggleMonth(key) {
        // Toggle: if currently collapsed (true or undefined), expand (false); otherwise collapse (true)
        collapsedMonths = { ...collapsedMonths, [key]: !isCollapsed(key) };
    }
...
<div class="card-base p-4">
    ...
    {:else}
        <!-- Scrollable container with max-height for progressive disclosure -->
        <div class="max-h-[400px] overflow-y-auto pr-1 scrollbar-thin">
            <div class="relative">
                <!-- Timeline line -->
                <div
