...
{#snippet list(choice, navigate)}
  ...
  <p class="mb-4 font-medium">{choice.mode === 'average' ? result.denominator ? `${result.total} across ${result.denominator} services = ${shown} per service${shown !== result.displayed ? ` (${result.displayed} before display rounding)` : ""}` : 'Average unavailable: no recorded services in this selection.' : `${result.total} across ${result.denominator} services`}</p>
  <ContributionList domain="service" records={result.rows.map(({ service, contribution }) => ({ id: serviceId(service), title: title(service), date: date(service.service_date), contribution, note: String(service.service_type || '').replaceAll('_', ' ') }))} onselect={row => navigate({ kind: 'service', title: row.title, id: row.id, metricKey: choice.metricKey }, row.id)} />
