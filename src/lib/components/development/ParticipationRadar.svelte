<script>
  function inspect(axis, personId) { onSelect?.({kind:'attendance', title:axis.label, personId, axisKey:axis.key, basis}); }
  let { axes, comparison = null, basis = 'weeks', selected = 'prayer', name = 'Person', compareName = '', personId = '', comparePersonId = '', onSelect = null } = $props();
...
<svg viewBox="0 0 340 300" role="img" aria-label="Attendance comparison radar">
  ...
  {#if other}{#if other.every(v => v !== null)}<polygon points={polygon(other)} class="other"/>{:else}<path d={segments(other)} class="other"/>{/if}{#each other as value,i}{#if value!==null}{@const p=point(i,value)}<circle cx={p.x} cy={p.y} r="4" class="other-point" role="button" tabindex="0" aria-label={`View Series B ${compareName} ${comparison[i].label} details`} onclick={() => inspect(comparison[i], comparePersonId)} onkeydown={event => { if (["Enter", " "].includes(event.key)) { event.preventDefault(); inspect(comparison[i], comparePersonId); } }}/>{/if}{/each}{/if}
  {#if values.every(v => v !== null)}<polygon points={polygon(values)} class="shape"/>{:else}<path d={segments(values)} class="shape incomplete"/>{/if}
  {#each values as value,i}{#if value!==null}{@const p=point(i,value)}<circle cx={p.x} cy={p.y} r={axes[i].key===selected?6:4} class="point" role="button" tabindex="0" aria-label={`View Series A ${name} ${axes[i].label} details`} onclick={() => inspect(axes[i], personId)} onkeydown={event => { if (["Enter", " "].includes(event.key)) { event.preventDefault(); inspect(axes[i], personId); } }}/>{/if}{/each}
</svg>
<style>
