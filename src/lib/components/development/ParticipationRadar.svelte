<script>
  ...
  function inspect(axis, personName) { detail = { title: axis.label, subtitle: personName, metrics: [{ label: basis === "weeks" ? "Weeks with attendance" : "Recorded attendances", value: axis[basis].attended }, { label: basis === "weeks" ? "Weeks with selected gatherings" : "Selected gatherings with known registers", value: axis[basis].offered }, { label: "Participation", value: percent(axis) === null ? "Unavailable" : `${Math.round(percent(axis))}%` }] }; }
...
<svg viewBox="0 0 340 300" role="img" aria-label="Attendance comparison radar">
  ...
  {#if other}{#if other.every(v => v !== null)}<polygon points={polygon(other)} class="other"/>{:else}<path d={segments(other)} class="other"/>{/if}{#each other as value,i}{#if value!==null}{@const p=point(i,value)}<circle cx={p.x} cy={p.y} r="4" class="other-point" role="button" tabindex="0" aria-label={`View ${compareName} ${comparison[i].label} details`} onclick={() => inspect(comparison[i], compareName)} onkeydown={event => { if (["Enter", " "].includes(event.key)) { event.preventDefault(); inspect(comparison[i], compareName); } }}/>{/if}{/each}{/if}
  {#if values.every(v => v !== null)}<polygon points={polygon(values)} class="shape"/>{:else}<path d={segments(values)} class="shape incomplete"/>{/if}
  {#each values as value,i}{#if value!==null}{@const p=point(i,value)}<circle cx={p.x} cy={p.y} r={axes[i].key===selected?6:4} class="point" role="button" tabindex="0" aria-label={`View ${name} ${axes[i].label} details`} onclick={() => inspect(axes[i], name)} onkeydown={event => { if (["Enter", " "].includes(event.key)) { event.preventDefault(); inspect(axes[i], name); } }}/>{/if}{/each}
</svg>
<style>
