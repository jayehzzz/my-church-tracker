...
<DashboardLayout>
  ...
  {:else}
    ...
    <div class="pb-10">
      ...
      <!-- Quick Actions List -->
      {#if !loading && filteredVisitations().length > 0}
        <div class="mt-4 card-base">
          ...
          <div class="space-y-2 max-h-[300px] overflow-y-auto scrollbar-thin">
            {#each filteredVisitations().slice(0, 10) as visitation}
              <div
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <div class="flex items-center gap-2">
                      ...
                      <span
                        class="text-xs px-2 py-0.5 rounded-full {getOutcomeClass(
                          visitation.outcome,
                        )}"
                      >
                        {formatOutcome(visitation.outcome)}
                      </span>
                      {#if visitation.follow_up_required}
                        <span
                          class="text-xs px-2 py-0.5 rounded-full bg-warning/10 text-warning"
                        >
                          Follow-up
                    ...
                    <div class="text-xs text-muted-foreground">
                      {formatDate(visitation.visit_date)} • By {visitation.visited_by_name ||
                        "—"}
                    </div>
                  </div>
                </div>
